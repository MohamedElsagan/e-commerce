...
const productController = {
  ...
  async create(req , res){
    try {
        ...
        const {img , price , translations} = req.body;
        const id = await ProductModel.createWithTranslation({img , price ,translations });
        const data = matchedData(req);
        
        console.log("✅ Product and translations added successfully");
        res.status(201).json({
